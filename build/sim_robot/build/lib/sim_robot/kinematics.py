...
# Swing schedule: (leg_ind, swing_index) - leg_ind swings during the
# swing_index-th slot of the cycle. Order of swing: BR, BL, FR, FL.
...
def shift_trajectory(leg_ind, xyzK):
    ...
    for swing_leg, swing_index in SCHEDULE:
        if leg_ind == swing_leg:
            xyzK_copy = xyzK.copy()
            xyzK_ind = 0
            # Phase-shift by whole swing slots. Use the actual trajectory
            # length so this stays correct now that the stall points are gone
            # (was hardcoded NUM_DATA_POINTS + 2*T_STALL).
            n = len(xyzK)
            shift = int(swing_index * (NUM_DATA_POINTS * SWING_FACTOR))
