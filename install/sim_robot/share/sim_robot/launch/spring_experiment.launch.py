...
Pick the configuration with the `spring` argument:
  ...
  ros2 launch sim_robot spring_experiment.launch.py spring:=plugin   # nonlinear FEA spring (plugin)

All three spawn the SAME robot the SAME way and use the SAME bridge (which
includes the /<leg>_<joint>/commanded_effort topics), so a baseline run and a
spring run are directly comparable. Every variant carries the
CommandedEffortPublisher, so the motor/PID effort — the quantity a parallel
spring actually reduces — is logged in all three.
...
# spring argument -> model file (all three include the effort publisher)
MODEL_BY_SPRING = {
    'none':   'model_effort.sdf',          # baseline + effort pub, no spring
    'native': 'model_spring_native.sdf',   # native linear spring + effort pub
    'plugin': 'model_spring_plugin.sdf',   # nonlinear plugin spring + effort pub
}
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description="Spring configuration: none | native | plugin"),
        DeclareLaunchArgument(
