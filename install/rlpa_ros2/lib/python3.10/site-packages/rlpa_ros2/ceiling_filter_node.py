    ...
    def callback(self, msg):
        ...
        for p in points:
            if p[2] < self.max_height:
                ...
                ceiling_points.append(p)

        # 3. Publish new cloud
        if not filtered_points:
            return

        # Remap Gazebo's sensor frame to the TF-published camera_link frame
        header = copy.copy(msg.header)
        header.frame_id = 'camera_link'

        new_msg = pc2.create_cloud_xyz32(header, filtered_points)
        new_msg2 = pc2.create_cloud_xyz32(header, ceiling_points)
        self.pub.publish(new_msg)
        self.pub2.publish(new_msg2)
